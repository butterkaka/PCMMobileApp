...
export class LiveGraphPage {
  ...
  addData() {
    ...
    try {
      if (!this.disabledDataFlag || this.toggleValue == Constants.messages.switchToVal) {
        // sensorUnitCheck()if()
        if ((this.liveGraphSetupDetails.channel >= Constants.channels.standaloneAnalog1FunctionChannel && this.liveGraphSetupDetails.channel <= Constants.channels.standaloneFunctionPT100Channel) && this.toggleValue == Constants.messages.switchToVal) {
          currentValueTemp = this.currentValue;
        }
    ...
    this.atmQuestionObjectList.forEach(element => {
      try {
        ...
        if (element.channel == this.liveGraphSetupDetails.channel && element.subChannel == this.liveGraphSetupDetails.subchannel) {
          ...
          if (this.toggleValue == Constants.messages.switchToVal && this.liveGraphSetupDetails.channel >= Constants.channels.standaloneAnalog1FunctionChannel && this.liveGraphSetupDetails.channel <= Constants.channels.standaloneFunctionPT100Channel) {
  ...
  */
  ...
  setGraphMinMax() {
    ...
    else if (this.liveGraphSetupDetails.channel >= Constants.channels.standaloneAnalog1FunctionChannel && this.liveGraphSetupDetails.channel <= Constants.channels.standaloneFunctionPT100Channel
      && this.toggleValue == Constants.messages.switchToVal) {
